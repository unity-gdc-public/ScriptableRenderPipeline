...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    ...
    public partial class HDRenderPipeline
    {
        ...
        static void RenderRaytracingDeferredLighting(CommandBuffer cmd, in DeferredLightingRTParameters parameters, in DeferredLightingRTResources buffers)
        {
            ...
            // Disable specular lighting if required
            if (parameters.disableSpecularLighting)
            {
                cmd.SetGlobalInt(HDShaderIDs._EnableSpecularLighting, 1);
            }
            ...
            // Restore the previous specular lighting flag
            if (parameters.disableSpecularLighting)
            {
                cmd.SetGlobalInt(HDShaderIDs._EnableSpecularLighting, 1);
            }
